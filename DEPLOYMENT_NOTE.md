...
商品发布和编辑会经过后端数据校验。标题、价格、分类、交易地点、卖家和状态都需要符合白名单或长度限制；分类只允许 `教材资料`、`数码电子`、`宿舍生活`、`运动户外`、`设计生专区`，状态只允许 `在售`、`已预定`、`已出`。图片上传只允许 jpg、png、webp 的 base64 data URL，base64 原始字符串最大约 2MB；超过限制或格式不允许时后端会返回 400，且不会上传到 Supabase Storage。
